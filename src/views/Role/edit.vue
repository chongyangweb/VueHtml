		...
		<div class="main_content">
			...
			<div v-for="v in parent" v-if="v.pid==0">
				<el-row :gutter="20" >
					...
					<el-col :span="21">
						<el-checkbox style="margin-bottom:10px;" :checked="checked(v.id)" :label="v.name" border size="small" :true-label="v.id" :false-label="-v.id" @change="change"></el-checkbox>
					    <el-checkbox style="margin-bottom:10px;" :label="vo.name" border :checked="checked(vo.id)" size="small" @change="change" :true-label="vo.id" :false-label="-vo.id" v-for="vo in parent" v-if="vo.pid==v.id"></el-checkbox>
					 </el-col>
				</el-row>
				<div class="unline2"></div>
			</div>
...
<script>
	export default {
	    ...
	    methods: {
	      submit:function(){
	      	var _this = this;
	      	this.$post(this.ROOT_URL + 'Admin/role/edit/' + this.$route.params.id ,{name:this.name,perssion:this.perssionStr}).then(function(res){
	      		_this.$message({
		          message: '恭喜你，修改成功！',
		          type: 'success'
		        });
	    ...
	    created:function(){
	    	var _this = this;
	    	this.$get(this.ROOT_URL + "Admin/role/edit/"+this.$route.params.id).then(function(res){
	    		_this.name = res.info.name;
	    		_this.parent = res.perssion;
	    		_this.perssionStr = res.info.perssion;
	    		_this.perssion = res.info.perssion.split(',');
	    		console.log(_this.perssion);
	    	});
	    }
	  };
</script>
