	...
	<div class="index_main">
		...
		<div class="unline"></div>
			...
			<el-table ref="multipleTable" :data="lists" tooltip-effect="dark" size="medium" style="width: 100%" @selection-change="changeFun">‘
				...
				<template slot-scope="scope"><el-tag type="info">{{ scope.row.get_grade == null ?'其他': scope.row.get_grade}}</el-tag></template>
				</el-table-column>

				<el-table-column label="科目" width="80">
				<template slot-scope="scope"><el-tag type="success">{{ scope.row.get_subject == null ?'其他': scope.row.get_subject.name}}</el-tag></template>
				</el-table-column>

				<el-table-column label="题目摘要" >
...
<script>
	...
	export default {
	    ...
	    methods: {
	      ...
	      search:function(){
	      	var _this = this;
	    	_this.$post(this.ROOT_URL + 'Admin/teacher_question_public/index',{limit:this.page.limit,page:this.page.page,title:this.title}).then(function(res){
	      ...
	      getList:function(){
	      	var _this = this;
	    	_this.$post(this.ROOT_URL + 'Admin/teacher_question_public/index',{limit:this.page.limit,page:this.page.page}).then(function(res){
	    		_this.lists = res.data;
	    		_this.page = res.page;
	    	});
	      }
