		...
		<div class="main_content">
			<el-tabs :tab-position="tabPosition" v-model="selects">
			    <el-tab-pane label="商品信息" name="first">
					...
					<el-row :gutter="20">
						<el-col :span="3"><div class="input_lable">栏目</div></el-col>
						<el-col :span="10">
					...
					<el-row :gutter="20">
						...
						<el-col :span="2"><el-button type="success" @click="nextto">下一步</el-button></el-col>
	...
	export default {
	    ...
	    methods: {
		      	...
		      	if(this.goods_id == ''){
	      			...
	      			this.$get(_this.ROOT_URL + "Admin/goods_spec/goods_spec_del/"+this.$route.params.id).then(function(res){
	      				_this.$get(_this.ROOT_URL + "Admin/goods_attr/goods_attr_del/"+_this.$route.params.id).then(function(res){
		      				_this.$post(_this.ROOT_URL + 'Admin/goods_attr/add',{goods_id:goods_id,name:guige,val:canshu}).then(function(res){
					      		_this.$message({
						          message: '恭喜你，规格添加成功！',
						          type: 'success'
						        });
	      ...
	      nextfo:function(){
	      	...
	      	if(spec_id != ''){
	      		this.$post(this.ROOT_URL + 'Admin/goods_spec/edit',{goods_id:goods_id,price:price,num:num,spec_id:spec_id}).then(function(res){
		      		_this.$message({
			          message: '恭喜你，SKU修改成功！',
			          type: 'success'
			        });
