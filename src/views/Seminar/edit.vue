<template>
	<div class="index_main">
		<div class="main_btn_left">
			...
			<div class="index_main_title">专题编辑</div>
		</div>
...
<script>
	export default {
	    ...
	    methods: {
	      submit:function(){
	      	var _this = this;
	      	this.$post(this.ROOT_URL + 'Admin/seminar/edit/' + this.$route.params.id ,{title:this.title,author:this.author,cid:this.cid,is_top:this.is_top,is_hot:this.is_hot,content:this.content,thumb:this.thumb}).then(function(res){
	      		_this.$message({
		          message: '恭喜你，修改成功！',
		          type: 'success'
		        });
