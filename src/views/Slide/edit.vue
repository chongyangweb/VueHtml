<template>
	<div class="index_main">
		<div class="main_btn_left">
			...
			<div class="index_main_title">幻灯片编辑</div>
		</div>
...
<script>
	export default {
	    ...
	    methods: {
	      submit:function(){
	      	var _this = this;
	      	this.$post(this.ROOT_URL + 'Admin/slide/edit/' + this.$route.params.id ,{name:this.name,image_url:this.image_url,url:this.url,sort:this.sort,is_type:this.is_type}).then(function(res){
	      		_this.$message({
		          message: '恭喜你，修改成功！',
		          type: 'success'
		        });
	    ...
	    created:function(){
	    	var _this = this;
	    	this.$get(this.ROOT_URL + "Admin/slide/edit/"+this.$route.params.id).then(function(res){
	    		_this.name = res.info.name;
	    		_this.url = res.info.url;
	    		_this.sort = res.info.sort;
	    		_this.is_type = res.info.is_type;
	    		_this.image_url = res.info.image_url;
	    		_this.fileList2.push({name:'',url:res.info.image_url});
